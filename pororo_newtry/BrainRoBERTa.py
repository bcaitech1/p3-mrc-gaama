# 원 경로
# /opt/conda/lib/python3.7/site-packages/pororo/models/brainbert/BrainRoBERTa.py
...
class BrainRobertaHubInterface(RobertaHubInterface):
    ...
    def predict_span(
        ...
        with torch.no_grad():
            ...
            # 여러 개 답 뽑도록 코드 수정
            # 토론 게시판의 서일님 코드 참조 http://boostcamp.stages.ai/competitions/31/discussion/post/315
            results = []

            top_n = 20 # 수정하는 것으로 출력 개수를 달리할 수 있습니다. start logit 10개, end logit 10개로 -> 총 100개의 값을 제시합니다.
            ...
            for start in starts:
                ...
                ends = (masked_ends+ends)[:10] # masked_ends가 비어있을 경우를 대비
                for end in ends:
                    ...
                    # score 변형
                    # 정답 후처리
                    score = ((logits[:,0][start]+5) * logits[:,1][end]).item()
                    if score >= 0 and answer != "" and len(answer) <= 80:
                        pos_answer = self.mecab.pos(answer)
                        for element in pos_answer:
